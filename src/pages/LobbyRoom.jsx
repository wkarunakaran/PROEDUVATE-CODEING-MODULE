...
export default function LobbyRoom() {
  ...
  const fetchLobby = async () => {
    try {
      ...
      setLoading(false);
      showToast("Lobby not found or has been closed", "error");
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-foreground text-2xl">Loading lobby...</div>
      </div>
    );
  }
  ...
  if (!lobby) {
    return (
      <div className="min-h-screen bg-background flex items-center justify-center">
        <div className="text-foreground text-2xl">Lobby not found</div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-background py-8 px-4">
      <div className="max-w-6xl mx-auto">
        {/* Header */}
        <div className="bg-card rounded-2xl shadow-2xl p-8 mb-6 border border-border">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
            <div>
              <h1 className="text-4xl font-bold text-foreground mb-2">{lobby.lobby_name}</h1>
            ...
            <div className="mt-4 md:mt-0">
              <div className="text-muted-foreground text-sm mb-1">Game ID</div>
              <div className="flex items-center gap-2">
                <div className="bg-purple-600/10 px-6 py-3 rounded-lg font-mono text-3xl font-bold text-foreground tracking-wider border-2 border-purple-500">
                  {gameId}
                </div>
                <button
                  onClick={copyGameId}
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
            <div className="bg-muted rounded-lg p-4">
              <div className="text-muted-foreground text-sm mb-1">Host</div>
              <div className="text-foreground font-semibold">{lobby.host_username}</div>
            </div>
            <div className="bg-muted rounded-lg p-4">
              <div className="text-muted-foreground text-sm mb-1">Players</div>
              <div className="text-foreground font-semibold">{playerCount} / {maxPlayers}</div>
            </div>
            <div className="bg-muted rounded-lg p-4">
              <div className="text-muted-foreground text-sm mb-1">Time Limit</div>
              <div className="text-foreground font-semibold">{lobby.time_limit_seconds / 60} min</div>
            </div>
            <div className="bg-muted rounded-lg p-4">
        ...
        <div className="bg-card rounded-2xl shadow-2xl p-8 border border-border">
          <h2 className="text-2xl font-bold text-foreground mb-6">Players in Lobby</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  ...
                  <div>
                    <div className="text-foreground font-semibold text-lg">
                      ...
                      {player.user_id === lobby.host_id && (
                        <span className="ml-2 text-yellow-600 dark:text-yellow-400"><Crown size={18} /></span>
                      )}
                      {player.user_id === currentUserId && (
                        <span className="ml-2 text-purple-600 dark:text-purple-500">(You)</span>
                      )}
                    </div>
                    <div className="text-muted-foreground text-sm">Player #{index + 1}</div>
                  </div>
            ...
            {Array.from({ length: maxPlayers - playerCount }).map((_, index) => (
              <div
                key={`empty-${index}`}
                className="p-4 rounded-lg border-2 border-dashed border-border bg-muted/50"
              >
        ...
        {/* Instructions */}
        <div className="mt-6 bg-blue-500/10 rounded-xl p-6 border border-blue-500/30">
          <h3 className="text-xl font-bold text-blue-600 dark:text-blue-500 mb-3 flex items-center gap-2">
            <Megaphone size={24} /> How to Invite Players
          </h3>
          <ol className="text-muted-foreground space-y-2 list-decimal list-inside">
            <li>Share the Game ID <span className="font-mono font-bold text-foreground">{gameId}</span> with your friends</li>
